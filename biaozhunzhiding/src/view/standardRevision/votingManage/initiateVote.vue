<template>
  <div class="content-wrapper">
    <div class="content-main">
      ...
      <el-button type="primary" @click="NewVote"  size="medium" style="margin-left:20px;">创建投票</el-button>
        <el-table :data="tableData" class="table1" style="width: 100%; margin-top: 10px">
          <el-table-column type="index" :index="typeIndex" prop="id" label="序号" width="60"></el-table-column>
          ...
          <el-table-column prop="voteStatus" label="投票状态" >
            <template slot-scope="scope">
              <template v-if="scope.row.voteStatus == '1'">
                已结束
              </template>
              <template v-if="scope.row.voteStatus == '2'">
                待启动
              </template>
              <template v-if="scope.row.voteStatus == '3'">
                进行中
              </template>
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" >
            <template slot-scope="scope">
              <template v-if="scope.row.voteStatus == '2'">
               <el-button type="text" size="small" @click="updateItemInfo(scope.row)">
                 <i class="icon iconfont icon-xiugai"></i> 编辑
               </el-button>
              </template>
...
<script>
  import {voteList} from 'api/standardAppliList/voteManage/vote_zj'
  export default {
    data(){
      return {
        // type: 133,
    ...
    methods: {
      ...
      searchList(){
        ...
        voteList(data).then(response => {
          this.tableData = response.data.list;
          this.formPage.total = response.data.total
        })
      },
      NewVote(){
        this.$router.push({path: '/initiateNewVote',query:{type : 'add'}});
      },
      showItemInfo(row){
        this.$router.push({name: "myVoteDetailView", query:{row:row,type:"initiateVote"}})
      },
      updateItemInfo(row){
        this.$router.push({name: "initiateNewVote", query:{id:row.id,type:"update"}})
      },
      initInfo(){
        let data = {
          "userId": this.userInfo.userId,
