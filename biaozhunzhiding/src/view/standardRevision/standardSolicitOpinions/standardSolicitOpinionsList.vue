    ...
    <div class="content-main">
      ...
      <div class="content-table">
        <el-table :data="tableData" class="table1" style="width: 100%; margin-top: 10px">
          ...
          <el-table-column prop="collectionMethod" label="征集方式" width="80">
               <template slot-scope="scope">
              <template v-if="scope.row && scope.row.collectionMethod == '1'">
                公开征集
              </template>
              <template v-if="scope.row && scope.row.collectionMethod == '2'">
                专家征集
              </template>
            </template>
          </el-table-column>
...
<script>
  ...
  import {SolicitOpinionsList,opiniontcReview} from 'api/standardAppliList/standardSolicitOpinions/standardSolicitOpinions'
  export default {
    // filters: {
    ...
    methods: {
      ...
      findInfo(){
          ...
          // console.log(data)
           SolicitOpinionsList(data).then(res=>{
      ...
      showItemInfo(row){
        ...
        this.$router.push({name: "standardSolicitOpinionsDetail", query:{projId,state,times,id,type:'show'}})
      },
      updateItemInfo(row){
         let { projId,state,times,id } = row
        this.$router.push({name: "standardSolicitOpinionsDetail", query:{projId,state,times,id,type:'update'}})
      },
      initInfo(){
        let data = {
        ...
        SolicitOpinionsList(data).then(response => {
