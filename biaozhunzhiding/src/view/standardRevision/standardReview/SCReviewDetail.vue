  ...
  <div class="content-wrapper">
    <div class="content-title">
      <el-button class="title-button" size="mini" @click="returnList">
        ...
        <span class="title-info">SC审批送审稿</span>
    </div>
    <div class="content-main">
      <el-tabs tab-position="left">
        <el-tab-pane label="业务办理">
          <el-form :model="ruleForm" ref="ruleForm" label-width="135px" class="demo-ruleForm mar-t-20">
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="编制说明:" prop="instructionsResId">
                    <div v-if="showType">
                    ...
                    <template v-else>
                      <el-upload class="upload-demo"
                        ...
                        :multiple='true'
                        :file-list="fileInstructionsResList"
                      >
              ...
              <el-col :span="20">
                <el-form-item label="会议纪要:" prop="meetingSummaryResId">
                    ...
                    <template v-else>
                        ...
                        :multiple='true'
                        :file-list="fileMeetingSummaryResList"
                        :before-upload="onBeforeUpload"
                        >
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="标准报批稿:" prop="approvalDraftResId">
                  <div v-if="showType">
                    <li v-for="item in manuscriptsResList" :key="item.resId" @click="downloadFile(item.resId)" style="text-decoration:underline;color: #409EFF;cursor: pointer;">
                      {{item.resName}}
                    </li>
                    <!--<li v-for="item in manuscriptsResList" :key="item.resId" style="text-decoration:underline;color: #409EFF;">-->
                      <!--<a target="_blank" :href="downloadFileUrl(item.resId)" style="text-decoration:underline;color: #409EFF;">{{item.resName}}</a>-->
                    <!--</li>-->
                  </div>
                  <template v-else>
                    <el-upload class="upload-demo"
                               ...
                               :multiple='true'
                               :file-list="filemanuscriptsResList"
                               :before-upload="onBeforeUpload"
                    >
                      <el-button :disabled="showType" type="primary">点击上传</el-button>
                    </el-upload>
          ...
          <el-form :model="ruleForm2" ref="ruleForm2" label-width="235px" class="demo-ruleForm mar-t-20">
            ...
            <el-row>
              <el-col :span="10">
                <el-form-item label="制定/修订:" prop="mendType">
                  <div class="list-width">
                    <template v-if="ruleForm2.mendType == '1'">
                      修订
                    </template>
                    <template v-else-if="ruleForm2.mendType == '0'">
                      制定
                    </template>
                  </div>
            ...
            <el-row>
              ...
              <el-col :span="10">
                <!-- <el-form-item label="ICS:" prop="ics"> -->
                  <!-- <div class="list-width">{{ruleForm2.ics}}</div> -->
                  <!-- <div  v-if="showType" class="list-width" >{{ruleForm2.ics}}</div>
               <el-input class="input-width" v-else-if="updateType" v-model="ruleForm2.ics"></el-input>
               <el-input class="input-width" v-else v-model="ruleForm2.ics"></el-input> -->
                <!-- </el-form-item> -->
              </el-col>
            </el-row>
            <el-row>
        ...
        <el-tab-pane label="投票结果">
                  ...
                  <el-row>
                    ...
                    <el-col :span="6">
                      <el-form-item label="不赞成 :">
                        <span>{{formLabelAlign.opposeCount}}</span>
                      </el-form-item>
                    </el-col>
                    <el-col :span="6">
                      <el-form-item label="弃权 :">
                        <span>{{formLabelAlign.abstainCount}}</span>
                      </el-form-item>
                    </el-col>
                    <el-col :span="6">
          ...
          <hr>
          <el-table :data="tableData3" border style="width: 100%;margin-top: 10px" class="table1">
      ...
      <el-form>
        <el-form-item v-show="!showType" class="btnGroup">
          ...
          <el-button type="primary" @click="submitForm">发送报批稿</el-button>
...
<script>
  ...
  // import {getRecordList,getProject,getSelectInfo} from 'api/standardAppliList/voteManage/vote'
  ...
  export default {
    data() {
      return {
        ...
        isFirst2:true,
        instructionsResList:[],
        fileInstructionsResList:[],
        meetingSummaryResList:[],
        fileMeetingSummaryResList:[],
        manuscriptsResList:[],
        filemanuscriptsResList:[],
        approvalDraftResList:[],
        fileapprovalDraftResList:[],
        ...
        ruleForm2: {},
        downloadUrl: voteUrl,
        fileUploadUrl: voteUrl+'resNew/fileUpload', //上传接口
        showType: false,
        updateType: false,
        radio2: 1,
        plateOptions: [],
    ...
    mounted() {
      this.$nextTick(function () {
        ...
        voteDartList(data).then(res => {
          ...
          this.tableData3 = res.data&&res.data.voteList?res.data.voteList:[]
        })
      })
    },
    methods: {
      ...
      handleRemove3(file, fileList) {
        const repla = this.ruleForm.manuscriptsResId.indexOf(`,${file.resId}`) >= 0
          ? `,${file.resId}`
          : this.ruleForm.manuscriptsResId.indexOf(',') >= 0 ? `${file.resId},` : file.resId;
        this.ruleForm.manuscriptsResId = this.ruleForm.manuscriptsResId.replace(repla, '')
      },
      ...
      handleSuccess3(response, file, fileList) {
        file.resId = response.data[0].resId;
        if (this.ruleForm.manuscriptsResId) {
          this.ruleForm.manuscriptsResId += `,${response.data[0].resId}`
        } else {
          this.ruleForm.manuscriptsResId = response.data[0].resId
        }
      },
      ...
      getInfo(val) {
        ...
        //获取办理业务
        scReviewListDetail(parseInt(this.$route.query.id)).then(res => {
          // console.log(res)
          if(res.data){
            const { instructionsResId,manuscriptsResId,meetingSummaryResId } = res.data;
            if(instructionsResId){
              this.getFileDetails(instructionsResId,(info)=>{
                this.instructionsResList=info;
                this.fileInstructionsResList=info.map(item=>{
                  return {
                    name:item.resName,
                    percentage:100,
                    resId:item.resId,
            ...
            if(manuscriptsResId){
              debugger;
              this.getFileDetails(manuscriptsResId,(info)=>{
                this.manuscriptsResList=info;
                this.filemanuscriptsResList=info.map(item=>{
                  return {
                    name:item.resName,
                    percentage:100,
                    resId:item.resId,
            ...
            if(meetingSummaryResId){
              this.getFileDetails(meetingSummaryResId,(info)=>{
                ...
                this.fileMeetingSummaryResList=info.map(item=>{
                  return {
                    name:item.resName,
                    percentage:100,
                    resId:item.resId,
      ...
      submitForm() {
        if (this.$route.query.voteStateName == '进行中') {
          this.$message.error('投票未结束');
          return;
        }
        let data;
        if (this.$route.query.id) {
          data = {
            ...
            "manuscriptsResId": this.ruleForm.manuscriptsResId,
            "meetingSummaryResId": this.ruleForm.manuscriptsResList,
            "operate": '2',
