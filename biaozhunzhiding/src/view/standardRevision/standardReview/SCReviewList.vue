    ...
    methods: {
      ...
      sendVote() {
        if(this.multipleSelection == ''){
          ...
          this.$router.push({name: 'initiateNewVote',query:{row : this.multipleSelection, type : 'addNew', standardType : 'SC'}})
        }
      },
      showItemInfo(row) {
        let {projId,state,times,id} = row
        this.$router.push({
          name: "SCReviewDetail",query: {projId,state,times,id,type: 'show'}
        })
      },
      reviewItemInfo(row) {
        // debugger
        let {projId,state,times,id,voteStateName} = row
        this.$router.push({
          name: "SCReviewDetail",query: {projId,state,times,id,voteStateName,type: 'review'}
        })
      },
      initInfo() {
        let data = {
        ...
        scReviewList(data).then(response => {
          // debugger;
