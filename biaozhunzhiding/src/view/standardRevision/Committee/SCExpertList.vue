<template>
  <div class="content-wrapper">
    <h3 class="h3"><i class="icon iconfont icon-liebiao"></i> SC专家管理列表</h3>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      add(){
        this.$router.push({name:'expertAdd',query:{type : 'add', returnType : 'SC'}})
      },
      showItemInfo(row){
        this.$router.push({name:'expertAdd2',query:{id:row.id, type : 'show', returnType : 'SC'}})
      },
      updateItemInfo(row){
        this.$router.push({name:'expertAdd1',query:{id:row.id,type : 'update', returnType : 'SC'}})
      },
      del(row){
