<template>
  <div class="content-wrapper">
    ...
    <div id="print" style="display:none">
        ...
        <div class="tableList">
            <table cellpadding="0" cellspacing="0">
                ...
                <tr>
                    <td class="th-title">任务进展</td>
                    <td class="th-title th-style" style="border:1px #ddd solid;">{{processStatusName}}</td>
                    <td class="th-title th-style" style="border-top:1px #ddd solid;">报送时间</td>
                    <td class="thingsTd" colspan="3" style="position: relative;">
                        <div class="list-width" style="text-align: center;">{{reportTime}}</div>
                        <!--<input type="text" name="text" style="border-top:1px #ddd solid; position: absolute;top:0px;" v-model="reportTime" disabled>-->
                    </td>
    ...
    <div class="button-last">
      ...
      <el-button type="primary" @click="doPrint" class="btn">打印</el-button>
  ...
  export default {
    ...
    created(){
        ...
        getWookSheet(data).then(response => {
            ...
            // console.log(response.data.hadlingResult);
    ...
    methods:{
     ...
     handleSuccess(response, file, fileList){
      ...
      returnList(){
        this.$router.push({name:'toDoManaged'});  /* 点击返回跳转 */
      },
      save(){
          let data = {
          ...
          progressReportTemporary(data).then(response => {
            if(response.code !== -1){
              ...
              this.$router.push({name:'toDoManaged'})
            }else{
      ...
      sendOffice(){
        if(this.processStatus==''){
          this.$message.warning('请选择任务进展');
          return
        }else if(this.hadlingStatus==''){
          this.$message.warning('请填写办理情况');
          return
        }else{
          let data = {
            "id":this.$route.params.wsId,
