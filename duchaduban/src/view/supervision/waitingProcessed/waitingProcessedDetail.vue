	    ...
	    <div class="tableList">
	      <table cellpadding="0" cellspacing="0">
	        <tr>
	          ...
	          <th class="th-title th-style">事件状态1111</th>
	          <td class="thingsTd">
	            <div class="list-width" style="text-align: center;">在办</div>
	            <!--<input type="text" name="text" value="办理中" disabled>-->
	          </td>
...
<script>
  import {getWookSheet,progressReport,progressReportTemporary, filesDetail, getReportLeader } from  'api/supervision/index'
  ...
  export default {
    ...
    methods:{
      ...
      returnList(){
        this.$router.push({name:'waitingProcessed'});  /* 点击返回跳转 */
      },
      save(){
          let data = {
          ...
          progressReportTemporary(data).then(response => {
            if(response.code !== -1){
              ...
              this.$router.push({name:'waitingProcessed'})
            }else{
